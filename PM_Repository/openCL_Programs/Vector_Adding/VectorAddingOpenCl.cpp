...
void create_kernels(utt_ocl_struct utv_ocl_struct)
{
    ...
    if (utr_ocl_GPU_context_exists(platform_index, utv_ocl_struct)) {
        ...
        utr_ocl_create_kernel_dev_type(platform_index, UTC_OCL_DEVICE_GPU, kernel_index,
            // kernel name:         , file:
            (char *)"OpenCL_VectorAdding_kernel", "VectorAdding.cl", monitor, utv_ocl_struct);
    }
    ...
    std::cout << "EXECUTION TIME: creating CPU kernel: " << duration3.count() << " nanosekund" << std::endl;
}

int OpenCL_Vector_Adding(
    int kernel_index,
    SCALAR* A,
    SCALAR* B,
    SCALAR* C,
    ...
    const cl_kernel OpenCL_Vector_Adding_Kernel,
...
)
{
    ...
    // Invoke kernel 
    retval = clSetKernelArg(OpenCL_Vector_Adding_Kernel, 0, sizeof(int), (void*)&N);
    retval |= clSetKernelArg(OpenCL_Vector_Adding_Kernel, 1, sizeof(cl_mem), (void*)&d_A);
    retval |= clSetKernelArg(OpenCL_Vector_Adding_Kernel, 2, sizeof(cl_mem), (void*)&d_B);
    retval |= clSetKernelArg(OpenCL_Vector_Adding_Kernel, 3, sizeof(cl_mem), (void*)&d_C);
    if (retval != CL_SUCCESS) {
    ...
    cl_event ndrEvt;
    clEnqueueNDRangeKernel(queue, OpenCL_Vector_Adding_Kernel, work_dim, 0,
...
#define VECTOR_SIZE 64*1024*1024*WORK_GROUP_SIZE
  ...
  // create arrays (vectors)
    ...
    for (idev = 0; idev < platform_struct.number_of_devices; idev++) {
        ...
        OpenCL_Vector_Adding(kernel_index, A, B, C, size_abc, context, kernel, command_queue);
        ...
        std::cout << "Czas wykonania: " << duration2.count() << " nanosekund\n" << " ( " << seconds2 << " seconds)" << std::endl;
...
int main()
{
    ...
    std::cout << "EXECUTION TIME: creating command queues: " << duration4.count() << " nanosekund" << std::endl;



             printf("WYBIK %d", sizeof(utv_ocl_struct.list_of_platforms) / sizeof(utv_ocl_struct.list_of_platforms[0]));
